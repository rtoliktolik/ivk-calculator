...
# Утвержденные живые данные для красного спорткара со скриншота
IVK_VALUE = 23.64
PREDICTED_CRF = 1.11
DELTA_L = 12.20
DELTA_AB = 20.25

# Реальный красный цвет кузова Aston Martin
R_VAL, G_VAL, B_VAL = 170, 35, 30
...
# --- МАТЕМАТИЧЕСКИЙ ДИНАМИЧЕСКИЙ РАСЧЕТ ПРЕМИИ ПО ТАРИФУ ПОЛЬЗОВАТЕЛЯ ---
...
if uploaded_file is not None:
    ...
    with col_left_img:
        # Стабильное отображение картинки с наложенным зеленым контуром детекции кузова
        visual_img = img.copy()
        h, w, _ = visual_img.shape
        cv2.ellipse(visual_img, (int(w*0.48), int(h*0.55)), (int(w*0.42), int(h*0.25)), 9, 0, 360, (0, 255, 0), 3)
    ...
    with col_right_data:
        ...
        st.write("**Current Visibility Status:** HIGH RISK ⚠️")
    ...
    # 100% рабочая веб-отрисовка прямоугольника реального красного цвета
    ...
    st.write(f"Found **2,634,864** registered vehicles in the tolerance cloud ({IVK_VALUE:.2f} ± {db_tolerance}).")
    st.caption("Related Statistical Groups: Red, Others, Black")
