...
def simulate_database_lookup(target_ivk: float, tolerance: float) -> dict:
    COLOR_STATS_DATABASE = [
        {"name": "Grey", "count": 3597270, "ivk_import streamlit as st
import cv2
import numpy as np
from ultralytics import YOLO
import os
...
st.markdown("""
    ...
    /* Стилизация вертикального ползунка через CSS без разрушения разметки */
...
if uploaded_file is not None:
    ...
    # Главная разметка страницы: Левая область с фото, Правая аналитическая панель
    ...
    with col_left_img:
        ...
        if manual_mode:
            ...
            # Пропорциональная сетка: Левая узкая колонка под слайдер Y, правая под фото
    ...
    if raw_dominant_color is not None:
        ...
        vm = float(va / 12.0)min": 0.0, "ivk_max": 25.0},
