...
def predict_crf_by_function(target_ivk: float) -> float:
    xp_arr = [12.5, 33.5, 47.0, 58.5, 80.0]
    fp_arr = [1.19, 1.03, 1.00, 0.975, 0.93]
...
if uploaded_file is not None:
    ...
    if manual_mode:
        ...
        with st.spinner("AI is isolating clean paintwork..."):
            ...
            if np.sum(car_mask) > 0:
                ...
                # ИСПРАВЛЕНО: Точне вилучення каналів BGR за індексами кортежу mean_bgr
    ...
    with col_left_img:
        ...
        # Компактне зображення автомобіля з шириною width=550
